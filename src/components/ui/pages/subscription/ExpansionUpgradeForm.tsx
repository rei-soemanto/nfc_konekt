...
export default function ExpansionUpgradeForm({ planId, monthlyExpansionPrice, remainingDays }: Props) {
    const router = useRouter();
    const [packsToAdd, setPacksToAdd] = useState(1);
    const [loading, setLoading] = useState(false);

    // --- PRO-RATED CALCULATION ---
    // Price per day = Monthly / 30
    // Total = PricePerDay * RemainingDays * Packs
    const pricePerDay = monthlyExpansionPrice / 30;
    const costPerPack = Math.ceil(pricePerDay * remainingDays);
    const totalCost = costPerPack * packsToAdd;
    
    const addedSeats = packsToAdd * EXPANSION_PACK_SIZE;

    const handleContinue = () => {
        setLoading(true);
        // ✅ FIX: Redirect to Team Setup first (Step 1 of 3)
        // Flow: Team Setup -> Design -> Payment
        ...
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-center text-sm text-gray-500 mt-3">
                    You will get <strong>+{addedSeats} Team Profiles</strong> valid for the next <strong>{remainingDays} days</strong>.
                </p>
            </div>

            {/* Bill Details */}
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-5 border border-dashed border-gray-300 dark:border-gray-700">
                <h3 className="font-bold text-gray-900 dark:text-white mb-4">Pro-Rated Summary</h3>
                
                <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                        <span className="text-gray-500">Base Monthly Rate</span>
                        <span className="font-mono text-gray-700 dark:text-gray-300">IDR {monthlyExpansionPrice.toLocaleString('id-ID')}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-gray-500">Remaining Duration</span>
                        <span className="font-mono text-gray-700 dark:text-gray-300">{remainingDays} Days</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                        <span className="text-gray-500">Cost per Pack (Pro-rated)</span>
                        <span className="font-mono text-gray-700 dark:text-gray-300">IDR {costPerPack.toLocaleString('id-ID')}</span>
                    </div>
            ...
            <button
                ...
                disabled={loading}
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white text-lg font-bold rounded-xl shadow-lg shadow-indigo-500/30 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
                {loading ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <><i className="fa-solid fa-users-gear"></i> Continue to Team Setup</>}
            </button>
        </div>
    )
}